...
import com.project.admin.StudentPreview;
...
public class EkitCore extends JPanel implements ActionListener, KeyListener,
	...
	JXDatePicker datePickerStart;
	JXDatePicker datePickerEnd;
	SimpleDateFormat formater;
	Date datedodc;
	private StudentPreview spreview;
	...
	private final String appName = "Add Message";
	...
	public EkitCore(boolean isParentApplet, String sDocument,
		...
		actionAlignLeft = new AlignAction(this,
				Translatrix.getTranslationString("AlignLeft"),
				StyleConstants.ALIGN_LEFT);
		actionAlignCenter = new AlignAction(this,
				Translatrix.getTranslationString("AlignCenter"),
				StyleConstants.ALIGN_CENTER);
		actionAlignRight = new AlignAction(this,
				Translatrix.getTranslationString("AlignRight"),
				StyleConstants.ALIGN_RIGHT);
		actionAlignJustified = new AlignAction(this,
		...
		JMenuItem jmiTable = new JMenuItem(
		...
		JMenuItem jmiEditTable = new JMenuItem(
		...
		JMenuItem jmiEditCell = new JMenuItem(
		...
		jMenuTable.addSeparator();
		JMenuItem jmiTableRow = new JMenuItem(
		...
		JMenuItem jmiTableCol = new JMenuItem(
		...
		jMenuTable.addSeparator();
		JMenuItem jmiTableRowDel = new JMenuItem(
		...
		JMenuItem jmiTableColDel = new JMenuItem(
	 ...
	 *            instead of paragraph tags.
	 */
	public EkitCore(boolean isParentApplet, String sRawDocument,
			URL urlStyleSheet, boolean includeToolBar, boolean showViewSource,
			boolean showMenuIcons, boolean editModeExclusive, String sLanguage,
			String sCountry, boolean base64, boolean hasSpellChecker,
			boolean multiBar, String toolbarSeq, boolean enterBreak) {
		this(isParentApplet, null, null, sRawDocument, (StyledDocument) null,
				urlStyleSheet, includeToolBar, showViewSource, showMenuIcons,
				editModeExclusive, sLanguage, sCountry, base64, false,
...

	{
		try {
			...
			if (command.equals(CMD_DOC_NEW)
				...
				SimpleInfoDialog sidAsk = new SimpleInfoDialog(this.getFrame(),
						"", true,
			...
			// Start/////////////////////////
				...
				final String[] list = { "Student", "Teacher" };
				String[] list2 = { " " };
				...
				jcb.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent event) {
						...
						if (jcb.getSelectedItem() == list[0]) {
							arr = greetingService.selshtml();
							jcb2.removeAllItems();
							for (int i = 0; i < arr.length; i++) {
								jcb2.addItem(arr[i]);

							}
						} else if (jcb.getSelectedItem() == list[1]) {
							arr = greetingService.selthtml();
							jcb2.removeAllItems();
							for (int i = 0; i < arr.length; i++) {
								jcb2.addItem(arr[i]);
				...
				datePickerStart.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						datedodc = datePickerStart.getDate();
				...
				datePickerEnd.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						datedodc = datePickerEnd.getDate();
				...
				Object[] jcb3 = { jcb, jcb2, start, datePickerStart, end,
						datePickerEnd };

				int reply = JOptionPane.showConfirmDialog(null, jcb3,
						"Select a value", JOptionPane.YES_NO_OPTION);

				if (reply == JOptionPane.YES_OPTION) {
					if (jcb.getSelectedItem() == list[0]) {
						System.out.println("1");
						greetingService.sendshtml(
								(String) jcb2.getSelectedItem(),
								jtpMain.getText(),
								formater.format(datePickerStart.getDate()),
								formater.format(datePickerEnd.getDate()));
					}

					else if (jcb.getSelectedItem() == list[1]) {
						System.out.println("2");
						greetingService.sendthtml(
								(String) jcb2.getSelectedItem(),
								jtpMain.getText(),
								formater.format(datePickerStart.getDate()),
								formater.format(datePickerEnd.getDate()));
				...
				// jcb.setSelectedItem(null);
				// jcb2.setSelectedItem(null);
				// JComboBox[] jcb3 = { jcb, jcb2 };
				// int reply = JOptionPane.showConfirmDialog(null, jcb3,
				// "Select a value", JOptionPane.YES_NO_OPTION);
				// if (reply == JOptionPane.YES_OPTION) {
				// updateTitle();
				// // try {
				// // // function.text(jtpSource.getText());
				// // } catch (SQLException e) {
				// // // TODO Auto-generated catch block
				// // e.printStackTrace();
				// // }
				// System.out.println(jtpSource.getText());
				// JOptionPane.showMessageDialog(null, "Send");
				// } else {
				//
				// }
				...
				String[] fieldNames = { "rows", "cols", "border",
						"cellspacing", "cellpadding", "width", "valign" };
				String[] fieldTypes = { "text", "text", "text", "text", "text",
						"text", "combo" };
				String[] fieldValues = { "3", "3", "1", "2", "4", "100%",
						"top,middle,bottom" };
				...
				new SimpleInfoDialog(this.getFrame(),
	...
	public JTextPane getJtpMain() {
		System.out.println("3 " + jtpMain.getText());
		return jtpMain;
	}
	...
	private void insertTable(Hashtable attribs, String[] fieldNames,
			String[] fieldTypes, String[] fieldValues) throws IOException,
			BadLocationException, RuntimeException, NumberFormatException {
		...
		if (fieldNames != null && fieldNames.length > 0) {
			PropertiesDialog propertiesDialog = new PropertiesDialog(
					this.getFrame(), fieldNames, fieldTypes, fieldValues,
	...
	public void insertURLImage() throws IOException, BadLocationException,
			RuntimeException {
		ImageURLDialog imgUrlDialog = new ImageURLDialog(this.getFrame(),
	...
	public void writeOutBase64(String text) throws IOException,
			BadLocationException {
		File whatFile = getFileFromChooser(".", JFileChooser.SAVE_DIALOG,
				extsB64, Translatrix.getTranslationString("FiletypeB64"));
		if (whatFile != null) {
	...
	private void getImageFromChooser(String startDir, String[] exts, String desc) {
		ImageFileDialog imgFileDialog = new ImageFileDialog(this.getFrame(),
				startDir, exts, desc, "",
	...
	public void dispose() {
		...
		frameHandler.setSize(100, 100);
		frameHandler.setResizable(false);
	}
